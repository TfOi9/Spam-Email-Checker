...
- 伪装100封垃圾邮件，随后进行测试：高伪装成功率的分类错误率100%（31/31），中伪装成功率的分类错误率62.5%（10/16），低伪装成功率的分类错误率27.3%（3/11），伪装失败的分类错误率2.4%（1/42），总错误率45%（45/100）。

三代模型：
- 错误率（判断垃圾邮件为正常邮件）为13.3%（186/1397）；
- 误判率（判断正常邮件为垃圾邮件）为5.99%（30/501）；